...
function HandScanEngine({ currentProfile, onScanCompleted, triggerFeedback }) {
  ...
  const startScan = async () => {
    ...
    try {
      ...
      const { FilesetResolver, HandLandmarker } = await import(
        ...
        throw new Error("MediaPipe CDN 로드 실패");
      });
      ...
      console.warn("MediaPipe init failed, using simulation mode:", err);
      setStatus("simulation");
      triggerFeedback("카메라 미사용 — 시뮬레이션 모드로 전환합니다.");
    }
  };
  ...
  const saveSnapshot = () => {
    ...
    onScanCompleted({ romDeg: avgFlexion, stiffnessMin, painIndex, recommendation: buildRecommendation(avgScore, avgFlexion) });
  ...
  const runSimulation = () => {
    ...
    onScanCompleted({ romDeg: simResult.romDeg, stiffnessMin: simResult.stiffnessMin, painIndex: simResult.painIndex, recommendation: buildRecommendation(simResult.avgScore, simResult.romDeg) });
  ...
  if (status === "simulation") return (
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-center">
        ...
        <p className="text-[10px] text-amber-600 mt-1">카메라 접근이 불가하여 시뮬레이션 데이터로 시연합니다.</p>
        <button onClick={runSimulation} className="mt-3 bg-teal-500 text-white font-bold text-xs px-4 py-2 rounded-xl">
          시뮬레이션 스캔 실행
        </button>
      </div>
    </div>
  ...
  return (
    <div className="space-y-3">
      <div className="relative w-full aspect-video bg-slate-950 rounded-2xl overflow-hidden border border-teal-500/30">
        ...
        <div className="absolute top-2 left-2 right-2 flex justify-between items-center z-20">
          <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${handDetected ? "bg-teal-500 text-slate-950" : "bg-slate-700 text-slate-400"}`}>
            {handDetected ? "손 감지됨" : "손을 화면에 보여주세요"}
          </span>
          <span className="text-[9px] text-teal-400 font-mono bg-slate-950/80 px-2 py-0.5 rounded-full">LIVE</span>
        </div>
        {liveMetrics && (
...
function TimelineModule({ currentProfile, triggerDoctorReportPrint, triggerFeedback }) {
  const painData = [
    {week:"1주차",pain:currentProfile.painIndex+2,rom:currentProfile.fingerScore-12},
    {week:"2주차",pain:currentProfile.painIndex+1,rom:currentProfile.fingerScore-8},
    {week:"3주차",pain:currentProfile.painIndex,rom:currentProfile.fingerScore-4},
    {week:"4주차",pain:Math.max(1,currentProfile.painIndex-1),rom:currentProfile.fingerScore},
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm">
        <p className="text-[10px] font-bold text-teal-700 mb-2">최근 4주간 통증 VAS 감소선</p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm">
        <p className="text-[10px] font-bold text-teal-700 mb-2">Finger Score™ 개선 추이</p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={painData}>
              ...
              <Bar dataKey="rom" name="Finger Score" radius={[4,4,0,0]}>
                {painData.map((_, i) => <Cell key={i} fill={i === 3 ? "#14b8a6" : "#99f6e4"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-2xl p-3 text-center">
        <p className="text-[10px] font-bold text-teal-800 mb-2">주간 회복 변화: <span className="text-teal-600">{currentProfile.weeklyROMChange}</span></p>
...
function JOINTRUNUnified() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f8fafc",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      {/* HEADER */}
      <header style={{background:"white",borderBottom:"1px solid #e2e8f0",padding:"12px 20px",position:"sticky",top:0,zIndex:50,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
        <div style={{maxWidth:"1280px",margin:"0 auto",display:"flex",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:"12px"}}>
          <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
            <div style={{background:"#0d9488",color:"white",padding:"8px 12px",borderRadius:"10px",display:"flex",alignItems:"center",gap:"6px",fontWeight:900}}>
              <Activity style={{width:18,height:18}} />
              <span style={{letterSpacing:"-0.5px"}}>JOINTRUN</span>
            </div>
            <div>
              <div style={{fontSize:15,fontWeight:800,color:"#0f172a",display:"flex",alignItems:"center",gap:"8px"}}>
                Movement Intelligence Platform
                <span style={{fontSize:10,color:"#0f766e",border:"1px solid #99f6e4",padding:"1px 8px",borderRadius:20,background:"#f0fdfa",fontFamily:"monospace"}}>통합 v2.0</span>
              </div>
              <div style={{fontSize:11,color:"#64748b"}}>MediaPipe HandScan + Anthropic AI Coach 통합</div>
            </div>
          </div>

          {/* Profile Selector */}
          <div style={{background:"#f8fafc",padding:"8px",borderRadius:"14px",border:"1px solid #e2e8f0",display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap"}}>
            <span style={{fontSize:11,color:"#64748b",fontWeight:600,display:"flex",alignItems:"center",gap:4}}>
              <User style={{width:13,height:13,color:"#f97316"}} />대상 환자:
            </span>
            <div style={{display:"flex",gap:4}}>
              {profiles.map(p => (
                <button key={p.id} onClick={() => { setSelectedId(p.id); setRecoverySteps(DEFAULT_STEPS); triggerFeedback(`${p.name} 님의 의학적 상황이 적용되었습니다.`); }}
                  style={{padding:"5px 10px",borderRadius:10,fontSize:11,fontWeight:700,border:"none",cursor:"pointer",background:selectedId===p.id?"#0d9488":"white",color:selectedId===p.id?"white":"#64748b",boxShadow:selectedId===p.id?"0 2px 6px rgba(13,148,136,0.3)":"0 1px 2px rgba(0,0,0,0.06)",transition:"all 0.2s"}}>
                  {p.name}
                </button>
              ))}
            </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:11,color:"#64748b"}}>{currentUser?.email}</span>
            <button onClick={logout} title="로그아웃"
              style={{display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700,color:"#64748b",background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"6px 10px",cursor:"pointer"}}>
      ...
      {/* MAIN LAYOUT */}
      <main style={{flex:1,maxWidth:"1280px",margin:"0 auto",width:"100%",padding:"20px 16px",display:"grid",gridTemplateColumns:"minmax(0,5fr) minmax(0,7fr)",gap:"20px",alignItems:"start"}}>

        {/* LEFT: PHONE EMULATOR */}
        <section style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
          <div style={{width:"100%",maxWidth:380,height:780,background:"white",borderRadius:48,border:"8px solid #0f172a",position:"relative",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 25px 50px rgba(0,0,0,0.12)",flex:"none"}}>

            {/* Notch */}
            <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:160,height:24,background:"#0f172a",borderRadius:"0 0 16px 16px",zIndex:40}} />

            {/* Status bar */}
            <div style={{background:"white",paddingTop:28,padding:"28px 20px 8px",display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:11,color:"#64748b",fontFamily:"monospace",borderBottom:"1px solid #f1f5f9",zIndex:30}}>
              <span style={{fontWeight:700,color:"#0f172a"}}>09:41</span>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{color:"#0d9488",fontWeight:700}}>● LTE</span>
                <div style={{width:20,height:10,border:"1px solid #cbd5e1",borderRadius:3,padding:1,display:"flex"}}>
                  <div style={{width:"100%",background:"#0d9488",borderRadius:2}} />
                </div>
              </div>
            </div>
            ...
            {/* Onboarding */}
            {showOnboarding && (
              <div style={{position:"absolute",top:68,left:12,right:12,background:"#0f172a",borderRadius:16,padding:14,border:"1px solid rgba(20,184,166,0.3)",zIndex:20,boxShadow:"0 8px 24px rgba(0,0,0,0.3)"}}>
            ...
            {/* App content */}
            <div style={{flex:1,overflowY:"auto",padding:"12px 14px 72px",background:"#f8fafc"}}>
              {activeTab === "home" && (
                <div>
              ...
              {activeTab === "progress" && <TimelineModule currentProfile={currentProfile} selectedProfileId={selectedId} triggerDoctorReportPrint={triggerDoctorReportPrint} triggerFeedback={triggerFeedback} />}
            ...
            {/* Bottom Nav */}
            <nav style={{position:"absolute",bottom:0,left:0,right:0,background:"white",borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",padding:"8px 8px 10px",zIndex:30,boxShadow:"0 -2px 10px rgba(0,0,0,0.06)"}}>
              {TAB_CONFIG.map(tab => (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id); triggerFeedback(tab.label); }}
                  style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"4px 0",background:"none",border:"none",cursor:"pointer",color:activeTab===tab.id?"#0d9488":"#94a3b8",fontWeight:activeTab===tab.id?800:400,transition:"color 0.2s",position:"relative"}}>
                  {tab.fab ? (
                    <div style={{width:36,height:36,background:"#f0fdfa",border:"1px solid #99f6e4",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",marginTop:-14,boxShadow:"0 4px 12px rgba(13,148,136,0.2)"}}>
          ...
          <p style={{fontSize:9,color:"#94a3b8",marginTop:10,textAlign:"center",maxWidth:320,lineHeight:1.6}}>
            * MediaPipe HandLandmarker + Anthropic AI Coach 실제 통합 데모. 카메라 권한 허용 시 실시간 관절 측정이 활성화됩니다.
          </p>
        </section>

        {/* RIGHT: INVESTOR SPEC DECK */}
        <section>
          <div style={{background:"white",borderRadius:20,border:"1px solid #e2e8f0",padding:20,boxShadow:"0 4px 16px rgba(0,0,0,0.06)"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,borderBottom:"1px solid #e2e8f0",paddingBottom:14,flexWrap:"wrap",gap:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <BookOpen style={{width:18,height:18,color:"#ea580c"}} />
                <div>
                  <div style={{fontSize:14,fontWeight:900,color:"#0f172a"}}>JOINTRUN Investor & Product Blueprint</div>
                  <div style={{fontSize:10,color:"#64748b"}}>통합 v2.0 — MediaPipe + Anthropic AI 기술 명세서</div>
                </div>
              </div>
              <input value={specSearch} onChange={e => setSpecSearch(e.target.value)} placeholder="설계서 검색..."
                style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"6px 12px",fontSize:11,color:"#0f172a",outline:"none",width:160}} />
            </div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:12,minHeight:480}}>
              {/* Left: section list */}
              <div style={{overflowY:"auto",maxHeight:520,display:"flex",flexDirection:"column",gap:4}}>
                {BLUEPRINT_SECTIONS.filter(s => s.title.includes(specSearch) || s.subtitle.includes(specSearch)).map(sec => (
                  <button key={sec.id} onClick={() => {
                    setActiveSpecSection(sec.id);
                    triggerFeedback(`설계서 JR-${String(sec.id).padStart(2,"0")} 로드`);
                    if ([1,2,3].includes(sec.id)) setActiveTab("home");
                    if ([5].includes(sec.id)) setActiveTab("scan");
                    if ([6].includes(sec.id)) setActiveTab("coach");
                    if ([7].includes(sec.id)) setActiveTab("health");
                  }}
                    style={{textAlign:"left",padding:"8px 10px",borderRadius:10,border:"1px solid",borderColor:activeSpecSection===sec.id?"#cbd5e1":"transparent",background:activeSpecSection===sec.id?"#f8fafc":"transparent",cursor:"pointer",transition:"all 0.15s",display:"flex",alignItems:"center",gap:6}}>
                    <span style={{fontSize:9,fontFamily:"monospace",background:"#f8fafc",color:"#ea580c",padding:"1px 5px",borderRadius:4,border:"1px solid #e2e8f0",fontWeight:700,whiteSpace:"nowrap"}}>
                      {String(sec.id).padStart(2,"0")}
                    </span>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:11,fontWeight:700,color:"#0f172a",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{sec.title}</div>
                      <div style={{fontSize:9,color:"#94a3b8",fontFamily:"monospace"}}>{sec.subtitle}</div>
                    </div>
                  </button>
                ))}
              </div>
              ...
              {/* Right: content */}
              <div style={{background:"#f8fafc",borderRadius:14,border:"1px solid #e2e8f0",padding:16,overflowY:"auto",maxHeight:520}}>
                {(() => {
                  const sec = BLUEPRINT_SECTIONS.find(s => s.id === activeSpecSection);
                  if (!sec) return null;
                  return (
                    <div>
                      <div style={{marginBottom:10}}>
                        <span style={{fontSize:9,fontFamily:"monospace",fontWeight:800,textTransform:"uppercase",letterSpacing:"0.1em",background:"#fff7ed",color:"#ea580c",border:"1px solid #fed7aa",padding:"2px 8px",borderRadius:20}}>
                          JR-{String(sec.id).padStart(2,"0")}
                        </span>
                      </div>
                      <h3 style={{fontSize:13,fontWeight:900,color:"#0f172a",marginBottom:4}}>{sec.title}</h3>
                      <p style={{fontSize:10,color:"#94a3b8",marginBottom:12,fontFamily:"monospace"}}>{sec.subtitle}</p>
                      <div style={{fontSize:11,color:"#334155",lineHeight:1.8,whiteSpace:"pre-line"}}>{sec.content}</div>
                      <div style={{marginTop:16,paddingTop:12,borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                        <span style={{fontSize:9,color:"#94a3b8"}}>에뮬레이터에서 <strong>{activeTab.toUpperCase()}</strong> 탭을 확인하세요.</span>
                        <div style={{display:"flex",gap:4}}>
                          <span style={{fontSize:9,background:"#f0fdfa",color:"#0d9488",padding:"2px 8px",borderRadius:20,border:"1px solid #99f6e4"}}>MediaPipe 통합</span>
                          <span style={{fontSize:9,background:"#fff7ed",color:"#ea580c",padding:"2px 8px",borderRadius:20,border:"1px solid #fed7aa"}}>YC Seed Ready</span>
                        </div>
                      </div>
                    </div>
                  );
  ...
  if (!currentUser) {
    return (
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f1f5f9",padding:16}}>
        <div style={{width:"100%",maxWidth:380,minHeight:520,background:"white",borderRadius:32,border:"8px solid #0f172a",overflow:"hidden",boxShadow:"0 25px 50px rgba(0,0,0,0.15)",display:"flex",flexDirection:"column"}}>
          <AuthScreen />
        </div>
      </div>
    );
