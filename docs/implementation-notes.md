...
# Implementation Notes — HOME Normal State 레이아웃 재배치 (2026-07-16)

## 배경

STEP 2 HOME 작업 중 지금까지는 카드 내용(진단성 카피 제거, ARO 카드 삭제 등)만 정리했고, 카드 "순서" 자체는 손대지 않았었다. Claude Design HOME Wireframe(Deliverable 3) 기준으로 순서를 맞춘다 — 요청받은 목표 순서: 오늘 상태 → 최근 변화 → Timeline 미리보기 → 오늘의 관찰 → 체크인/스캔.

## 컴포넌트 매핑

실제 Deliverable 3 파일 없이 요청 문구만으로 진행해, 아래 매핑은 컴포넌트명·기존 표시 문구 기준으로 추론한 것이다 — 어긋난 부분이 있으면 알려주면 바로 조정 가능:

| 요청한 슬롯 | 매핑한 컴포넌트 | 근거 |
|---|---|---|
| 오늘 상태 | `TodayStatusCard` | 파일명이 "Today/Status"와 정확히 대응 |
| 최근 변화 | `RelativeChangeCard` | 기존 표시 타이틀이 "최근 변화" |
| Timeline 미리보기 | `RecentTimelinePreview` | 기존 표시 타이틀이 "최근 Timeline" |
| 오늘의 관찰 | `PatternInsightCard` | 기존 표시 타이틀이 "패턴 관찰" |
| (명시 안 됐지만 유지) | `TodayActionCard` | "오늘의 관찰" 클러스터 바로 뒤에 원래 있던 위치를 그대로 유지 |
| 체크인/스캔 | `RecordSection`(체크인·회복 미션, 그대로 둠) + Scan 버튼(최하단, Secondary 톤) | 요청대로 회복 미션 체크리스트는 내부 순서 변경 없음 |

## 변경 내역

- [`src/components/tabs/HomeModule.jsx`](../src/components/tabs/HomeModule.jsx): 카드 순서를 `TodayStatusCard → RelativeChangeCard → RecentTimelinePreview → PatternInsightCard → TodayActionCard → RecordSection`으로 재배치. 맨 아래에 `JTButton variant="outline"`으로 스캔 버튼 추가(기존 RecordSection의 "기록 추가" 버튼과 동일한 Secondary 톤 컨벤션 재사용).
- [`src/components/JOINTRUNShell.jsx`](../src/components/JOINTRUNShell.jsx): 최상단 Primary 스캔 버튼("30초 스캔 시작하기", 파란 배경)을 `scanCount === 1`(FirstScanHomeState) 분기 안으로만 한정 — Normal 상태(HomeModule)에서는 이제 최하단 Secondary 버튼 하나만 존재. `RecentTimelinePreview`도 HomeModule 내부로 옮겼으므로, FirstScanHomeState 분기에서만 별도로 렌더한다(그 상태의 기존 동작은 변경 없음 — 요청 범위가 Normal 상태 재배치였으므로 First Scan 상태는 건드리지 않았다).

## 이 커밋에 대한 참고

이 작업은 원래 `feature/step2-cleanup-and-monthly-report` 브랜치(커밋 `6757e51`)에서 먼저 했었는데, PR #15가 그보다 먼저 머지되면서 main에 들어가지 못하고 남아 있었다. `feature/p0-b2c-safety-cleanup`이 main과 병합된 뒤(ARO 카드·소견서 모달이 완전 삭제된 상태) 그 위에 동일한 재배치 로직을 다시 적용했다 — 원래 커밋을 그대로 cherry-pick하면 이미 삭제된 코드 주변과 충돌하므로, 같은 결과가 나오도록 수동으로 재현했다.

## 검증

- `npm run lint` 0 errors, `npm run build` 성공, `npm test` 통과(전부 기존 테스트 그대로 — 이 변경은 로직이 아니라 렌더 순서만 바꾼 것이라 새 테스트는 추가하지 않음).
- 브라우저 확인: Empty State(스캔 0회)는 정상 렌더·콘솔 에러 없음 확인. **다만 Normal 상태(스캔 2회 이상)는 이 환경에 카메라 접근이 없어 실제로 화면에 띄워 확인하지 못했다** — 이번 세션의 Report 월간 콘텐츠 검증 때와 동일한 제약. 코드 리뷰 기준으로는 기존에 개별적으로 이미 정상 동작이 확인된 컴포넌트들을 새 순서로 배치만 한 것이라 위험은 낮다고 판단했지만, 실기기/카메라가 있는 환경에서 한 번 더 확인을 권장한다.
